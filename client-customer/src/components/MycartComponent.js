    ...
    const mycart = this.context.mycart.map((item, index) => {
      return (
        <tr key={item.product._id} className="datatable">
          ...
          <td>
            <span className="link" onClick={() => this.lnkRemoveClick(item.product._id)}>Xóa</span><br></br>
            <span className="link" style={{textDecoration:'underline'}}><Link to='/'>Thêm</Link></span>
          </td>
        </tr>
      );
    });
